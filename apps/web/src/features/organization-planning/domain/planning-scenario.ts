...
} from "../events/planning-domain-event"
import {
  createPlanningDomainEvent,
} from "../events/planning-domain-event"
import {
  incrementVersion,
} from "../services/increment-version"
import {
...
export class PlanningScenario {
  ...
  static restore(
    input: PlanningScenarioContract
  ) {
    assertPlanningDomain(
      PLANNING_SCENARIO_STATUSES.includes(
        input.status
      ),
      "invalid_input",
  ...
  get baseSnapshotId() {
    return this.props.baseSnapshotId
  }
  ...
  updateDetails(
    ...
    PlanningStateMachine.assertEditable(
      this.status
    )
  ...
  archive(occurredAt: Date) {
    PlanningStateMachine.assertArchivable(
      this.status
    )
  ...
  private transition(
    ...
    PlanningStateMachine.assertTransition(
      this.status,
      status
    )
  ...
  private copy(
    changes:
      Partial<PlanningScenarioContract>,
