...
const table = readFileSync(
  new URL("./employee-table.tsx", import.meta.url),
  "utf8"
)
...
test("People derives invitation role options from the server-resolved actor role", () => {
  assert.match(
    page,
    /currentUser[\s\S]*getCurrentCompanyContext\(\)/
  )
  assert.match(page, /getInvitationRoleOptionsForActor\([\s\S]*currentUser\.role/)
  assert.match(
    page,
...
test("table offers the action only for an obvious eligible existing Person", () => {
  ...
  assert.match(table, /!employee\.has_user_access/)
