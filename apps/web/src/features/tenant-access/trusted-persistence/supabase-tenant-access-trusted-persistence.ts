...
function failureFromError<T>(error: unknown): TenantAccessApplicationResult<T> {
  ...
  if (!code || !STABLE_CODES.has(code)) return { status: "unexpected_persistence_failure", code: "TENANT_ACCESS_PERSISTENCE_FAILED" }
...
function mapEnvelope<T>(data: unknown, validResult: (value: unknown) => value is T): TenantAccessApplicationResult<T> {
  ...
  if ((envelope.status === "conflict" || envelope.status === "denied" || envelope.status === "known_failure") && typeof envelope.code === "string" && STABLE_CODES.has(envelope.code)) {
    return { status: envelope.status, operationId: envelope.operationId, code: envelope.code }
  }
  return { status: "unexpected_persistence_failure", code: "TENANT_ACCESS_INVALID_RESULT" }
}

const isInvitation = (value: unknown): value is InvitationPersistenceResult =>
  isRecord(value) && typeof value.invitationId === "string" && (value.status === "pending" || value.status === "revoked")
...
const isRoleChange = (value: unknown): value is MembershipRolePersistenceResult =>
  isRecord(value) && typeof value.membershipId === "string" && typeof value.role === "string"
