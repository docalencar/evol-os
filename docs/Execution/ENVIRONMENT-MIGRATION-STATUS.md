...
Canonical Review is provisioned and validated through `0133`. Any later schema
promotion remains a separate, explicitly authorized operation under the canonical
governance policy — publishing migration source is never permission to apply it
remotely.

**Evidence basis.** The `0133` row rests on the governed D-R2 promotion: a
canonical, merged runner, its read-only PRE against Review's own migration ledger,
a single authorized mutation and a POST that verified the object and proved every
security fingerprint unchanged. Per-migration detail sections exist through `0121`
only; `0122 → 0133` are attested by that PRE history read plus the versioned
promotion runners committed under `scripts/review/`, and no per-migration section
is fabricated for them.

**Production and Legacy were not inspected** for this update and their rows are
carried forward unchanged. `UNKNOWN / REVERIFY BEFORE USE` still means exactly
that; Review being current never advances Production.
