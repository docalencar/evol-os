...
create unique index if not exists departments_active_name_company_uidx
  on public.departments(company_id, lower(btrim(name)))
  where deleted_at is null;

create unique index if not exists departments_active_code_company_uidx
  on public.departments(company_id, lower(btrim(code)))
