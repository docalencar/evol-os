-- MVP Closure: Recruitment Phase 2 — allow the job-opening entity timeline.
--
-- get_tenant_entity_activity_timeline_v1 whitelists entity_type to the
-- People/Organization entities, so the recruitment job-opening detail page's
-- timeline raised ACTIVITY_ENTITY_TYPE_INVALID (and previously fell back to a
-- direct activity_events read → 42501). This is a minimal, additive, forward-only
-- CREATE OR REPLACE that adds 'job_opening' to the allowed entity types.
--
-- Everything else is unchanged: identical signature and return shape, the same
-- membership gate (is_company_member), company scoping, SECURITY DEFINER, hardened
-- search_path, and execute grants. No table grants, no RLS/policy changes. The
-- job_opening branch uses the generic (non-person) entity match; recruitment
-- activity rows already carry entity_type='job_opening' (migrations 0093/0094).
...
begin
  ...
  if p_entity_type is null
    or p_entity_type not in ('department', 'team', 'position', 'person', 'job_opening') then
