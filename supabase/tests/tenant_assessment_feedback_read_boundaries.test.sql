...
select is((select proargnames from pg_proc where oid=signature::regprocedure),expected,description) from (values
 ...
 ('public.get_tenant_assessment_template_structure_v1(uuid,uuid)',array['p_company_id','p_template_id','record_type','record_id','parent_id','name','description','instructions','assessment_type','status','icon','color','weight','display_order','question','help_text','question_type','scale_min','scale_max','required','active','competency_id','competency_name']::text[],'assessment structure exact shape'),
 ...
 ('public.get_assessment_evaluator_workspace_v1(uuid,uuid)',array['p_company_id','p_response_id','record_type','record_id','parent_id','template_id','cycle_id','employee_id','evaluator_id','status','name','description','instructions','assessment_type','icon','color','weight','display_order','question','help_text','question_type','scale_min','scale_max','required','active','started_at','completed_at','submitted_at','competency_id','competency_name']::text[],'assessment evaluator workspace exact shape'),
...
from (values
 ...
 ('public.get_tenant_assessment_template_structure_v1(uuid,uuid)',array['uuid','uuid','text','uuid','uuid','text','text','text','text','text','text','text','numeric','integer','text','text','text','integer','integer','boolean','boolean','uuid','text']::text[],'assessment structure exact types'),
...
insert into public.assessment_responses(id,company_id,assessment_cycle_id,assessment_template_id,
  assessment_execution_snapshot_id,employee_id,evaluator_id,status,perspective) values
 ('88000000-0000-4000-8000-000000000801','88000000-0000-4000-8000-000000000101','88000000-0000-4000-8000-000000000601','88000000-0000-4000-8000-000000000301','88000000-0000-4000-8000-000000000751','88000000-0000-4000-8000-000000000206','88000000-0000-4000-8000-000000000205','submitted','legacy_unknown');
